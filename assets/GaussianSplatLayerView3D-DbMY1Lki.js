...
}`)}function Xt(s){s.code.add(C`vec4 unpackColor(uvec4 packedGaussian) {
vec4 color;
...
return color / 255.0;
}`),s.code.add(C`vec3 unpackScale(uvec4 packedGaussian) {
...
}`),s.code.add(C`const uint MASK_9_BITS = 0x1FFu;
...
return quaternion;
}`),s.code.add(C`vec3 unpackTileOriginRelativePosition(uvec4 packedGaussian) {
...
}`),s.code.add(C`vec3 unpackCameraRelativeGaussianPosition(uvec4 packedHeader, highp vec3 position, vec3 cameraPosition, vec3 cameraPos8k, vec3 cameraDelta) {
...
}`)}function Yt(s){s.code.add(C`mat3 quaternionToRotationMatrix(vec4 q) {
float x2 = q.x + q.x;
float y2 = q.y + q.y;
float z2 = q.z + q.z;
float xx = x2 * q.x;
